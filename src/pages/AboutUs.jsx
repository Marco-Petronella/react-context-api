...
export default function AboutUs() {
    ...
    return(
        ...
        {team.map((member) => (
            <div key={member.id} className={`card col-md-6 col-lg-4 p-2 g-2 ` + (mode === "light" ? ("") : ("dark-mode"))}>
              <img src={member.image} className="card-img-top object-fit-fill" alt={member.name} />
              <div className="card-body">
                <h5 className="card-title">{member.name}</h5>
                <h6 className={`card-subtitle mb-2 `} >Potere combattivo: {member.ki}-{member.maxKi}</h6>
